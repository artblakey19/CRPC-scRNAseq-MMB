...
###XPO signaling
#Heatmap for XPO signaling
Idents(object = mCRPC) <- "ARNE"
DefaultAssay(mCRPC) <- "RNA"
mCRPC <- ScaleData(mCRPC, features = rownames(mCRPC))
tiff(file = "mCRPC ARNE XPO RP family selected.tiff", width =5.5, height = 3.5, units = "in", compression = "lzw", res =200)
DoHeatmap(mCRPC, features = c("MET","XPO1", "XPO7",
                              "RPL11", "RPLP2",  "RPL21",
                              "RPS20", "RPS29",  "RPS27",  
                              "EIF4EBP1", "EIF5B", "EIF1B"
), draw.lines = TRUE, size = 3, disp.max = 1.3, disp.min = -1.1) + scale_fill_gradientn(colors = c("purple", "black", "yellow"), na.value = "white")
dev.off()
...
all.genes <- rownames(mCRPC)
mCRPC <- ScaleData(mCRPC, features = all.genes)
DNPCvARPC.Markers <- FindMarkers(mCRPC, ident.1 = c("DNPC"), 
...
all.genes <- rownames(mCRPC)
mCRPC <- ScaleData(mCRPC, features = all.genes)
NEPCvARPC.Markers <- FindMarkers(mCRPC, ident.1 = c("NEPC"), 
                                 ident.2 = c("ARPC"), min.pct = 0.01, logfc.threshold = 0.01)
...
all.genes <- rownames(mCRPC)
mCRPC <- ScaleData(mCRPC, features = all.genes)
DNPCvNEPC.Markers <- FindMarkers(mCRPC, ident.1 = c("DNPC"), 
                                 ident.2 = c("NEPC"), min.pct = 0.01, logfc.threshold = 0.01)
write.csv(DNPCvNEPC.Markers, "DNPCvNEPC.Markers.csv")

#p.adjust
DEG_DNPCvNEPC <- read.csv("DNPCvNEPC.Markers.csv") 
...
setwd("//isi-dcnl/user_data/zjsun/group/DO NOT USE-Lab Members/Won Kyung Kim/Human PCa/SU2C_PCF")
countdata <- read.table(file = "SU2C_PCF_CRPC_mRNAseq_polyA.csv", sep = ",", header = T, row.names=1, as.is=T)
...
mCRPC_count <- mCRPC@assays$RNA@counts
a <- as.matrix(mCRPC_count)
#Log Transform
b=log2(a/10+1)
c=log2(a+1)
...
b['MET', ]
c['MET', ]
mCRPC_count_log2 <- as.data.frame(c)
mCRPC_count_log2_1 <- as.matrix(mCRPC_count_log2)
