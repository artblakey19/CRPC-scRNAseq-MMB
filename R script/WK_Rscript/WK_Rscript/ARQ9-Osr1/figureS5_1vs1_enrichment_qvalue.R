...
format_method_name = function(methods){
  ...
  methods[methods == "pair_c_q"] = 'qvalue-pair-mis'
...
grid_ls = expand.grid(list(N, scenario_tot, dist_tot), stringsAsFactors = F)
...
for(i in 1:nrow(grid_ls)){
  ...
  dat = fdppow[[which(N %in% prop)]]
  
  ####### clipper variants #####
  ylim_fdr = 0.10
  method_subset = c('Clipper', 'qvalue-pair-mis', 'qvalue-pair-2as1', 'qvalue-pair-c', 'qvalue-pool')
...
pdf(file =  'lineplot_enrichment_singlerep_qvalue.pdf', height = 23.4, width = 15.3)
p = arrangeGrob(
  ...
  blankPlot,textGrob(label = 'd=1000                 d=10000',hjust = 0.4, gp = gpar(fontsize= textsize)),blankPlot,textGrob(label = 'd=1000                 d=10000',hjust = 0.5, gp = gpar(fontsize= textsize)),
