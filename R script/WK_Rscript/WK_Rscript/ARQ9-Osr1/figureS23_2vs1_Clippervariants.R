remove(list = ls())
setwd('../data_simulation/2vs1_diff/')
library(openxlsx)
library(scales)
library(ggplot2)
library(wesanderson)
...
method_tot = c('Clipper-diff-GZ(h=1)','Clipper-diff-GZ(h=2)',
               'Clipper-max-GZ(h=1)', 'Clipper-max-GZ(h=2)',  'Clipper-powerful',
...
col_palette = c(col_clipper_diff_1ko, col_clipper_diff,
                col_clipper_max_1ko, col_clipper_max, col_clipper_powerful,
...
pt_shape = c(rep(2,2),rep(3,2), 4, rep(5,2), rep(6,2), 7:10)
...
format_method_name = function(methods){
  ...
  methods[methods == "clippermax_1ko"] = 'Clipper-max-GZ(h=1)'
  methods[methods == "clipperdiff_1ko"] = 'Clipper-diff-GZ(h=1)'
  methods[methods == "clippermax"] = 'Clipper-max-GZ(h=2)'
  methods[methods == "clipperdiff"] = 'Clipper-diff-GZ(h=2)'
...
out_prop = c(0, 0.1)
analysis_tot = c('1sided', '2sided')

grid_ls = expand.grid(list(analysis_tot, scenario_tot, dist_tot), stringsAsFactors = F)
...
for(i in 1:nrow(grid_ls)){
  ...
  scenario = grid_i[2]
  dist = grid_i[3]
  fdppow = readRDS(file = paste0('sim_',analysis,'_unequal_',dist,'_',scenario,'.rds'))
  dat = fdppow[[which(out_prop %in% 0)]]
  
  ####### clipper variants #####
  ylim_fdr = 0.10
  #method_subset  = c( 'Clipper-powerful',  'BH-pool', 'BH-pair-2as1','BH-pair-mis',
  #                   'locfdr-emp','locfdr-perm')
  method_subset =  c('Clipper-max-GZ(h=1)', 'Clipper-max-GZ(h=2)', 
                   'Clipper-diff-GZ(h=1)', 'Clipper-diff-GZ(h=2)')
  ...
  if(i == 1){
    p = p + theme(legend.position = 'right',
                  legend.text = element_text(size = 0.9*textsize),
...
pdf(file =  'lineplot_2vs1_clippervariants.pdf', height = 22, width = 15.3)
p = arrangeGrob(
  ...
  blankPlot,textGrob(label = 'Enrichment              Differential',hjust = 0.4, gp = gpar(fontsize= textsize)),blankPlot,textGrob(label = ' Enrichment              Differential',hjust = 0.5, gp = gpar(fontsize= textsize)),
