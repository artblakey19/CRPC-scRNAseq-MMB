...
clipper1sided = function(score_exp,
  ...
  contrastScore_method = match.arg(contrastScore_method, choices = c('diff', 'max'), several.ok = F)
  importanceScore_method = match.arg(importanceScore_method, choices = c('diff', 'max'), several.ok = F)
  FDR_control_method = match.arg(FDR_control_method, choices = c('BC','BH','GZ'), several.ok = F)
...
compute_taukappa = function(score_exp, score_back, r1, r2, if2sided,
  ...
  imp_ls = sapply(perm_idx, function(x){
    ...
    if( importanceScore_method == "t"){
      imp = t <- sapply(1:nrow(se), function(j){
        if ( length(unique(se[j,])) == 1 | length(unique(sb[j,])) == 1) {
          return(0)
          
          } else {
            return(t.test(se[j,!(is.na(se[j,]))], sb[j,!(is.na(sb[j,]))], alternative = "two.sided", var.equal = F)$statistic)}})
    }else{
