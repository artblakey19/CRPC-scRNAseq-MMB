...
sim_multirep_norm_homo <- lapply(1:length(out_prop), function(i_out){
  ...
  fdppow_ls <- mclapply(1:iter, function (it){
    ...
    ##### use FDP-pooled
    fdr_pooled = compute_pooled_fdr(back = rowMeans(score_back, na.rm = T), exp = rowMeans(score_exp, na.rm = T))
    ...
    ##### use FDP-pair
    fdr_pooled = compute_pair_fdr(back = rowMeans(score_back, na.rm = T), exp = rowMeans(score_exp, na.rm = T))
    discovery_ls = find_discovery_wfdr(fdr = fdr_pooled, q = q_tot)
    fdppow_pair_fdp = compute_fdppow(discovery_ls = discovery_ls, trueidx = trueidx)
    ...
    discovery_ls = lapply(re_clipper$results,'[[', 'discovery')
    fdppow_clipper_2ko = compute_fdppow(discovery_ls = discovery_ls, trueidx = trueidx)
    ##### clipper with one-sided knockoff
    re_clipper <- clipper1sided(score_exp = score_exp, 
                          score_back = score_back, 
                          FDR = q_tot, 
                          ifuseknockoff = T,
    ...
    discovery_ls = lapply(re_clipper$results,'[[', 'discovery')
    fdppow_clipper_1ko = compute_fdppow(discovery_ls = discovery_ls, trueidx = trueidx)
    ...
    pow = list(pooled = fdppow_pooled[2,], 
               ...
               clipper_2sided_ko = fdppow_clipper_2ko[2,],
               clipper_1sided_ko = fdppow_clipper_1ko[2,],
...
saveRDS(sim_multirep_norm_homo, 'sim_multirep_norm_homo.rds')
...
saveRDS(sim_multirep_norm_hete, 'sim_multirep_norm_hete.rds')
...
saveRDS(sim_multirep_pois_homo, 'sim_multirep_pois_homo.rds')
...
saveRDS(sim_multirep_pois_hete, 'sim_multirep_pois_hete.rds')
...
saveRDS(sim_multirep_nb_homo, 'sim_multirep_nb_homo.rds')
...
saveRDS(sim_multirep_nb_hete, 'sim_multirep_nb_hete.rds')
