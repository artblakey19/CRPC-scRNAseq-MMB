...
for(i in 1:nrow(grid_ls)){
  ...
  scenario = grid_i[2]
  dist = grid_i[3]
  fdppow = readRDS(file = paste0('sim_2sided_',dist,'_',scenario,'_t.rds'))
  dat = fdppow[[which(out_prop %in% out)]]
  if (!(dist == 'norm' &scenario == 'hete')){
    fdppow2 = readRDS(file = paste0('../sim_2sided_',dist,'_',scenario,'.rds'))
...
pdf(file =  'lineplot_differential_t.pdf', height = 23.4, width = 15.3)
