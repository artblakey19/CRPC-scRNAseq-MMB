...
compute_paired_pval_pois_mis = function(exp, back, sd = 1){
  ...
  if (p > 0.5) {return(2 * (1-p))}else{return(2*p)}
}
...
mean_back = rep(0, n*1)
mean_exp = rep(0, n*2)
mean_exp[1:(length(positiveid)*2)] = rep(rnorm(length(positiveid), mean = 5, sd = 1), each = 2)
mean_exp[(length(negativeid)*2+1):(length(trueidx)*2)] = rep(rnorm(length(negativeid), mean = - 5, sd = 1), each = 2)
...
sim_2sided_norm_homo <- lapply(1:length(out_prop), function(i_out){
  ...
  fdppow_ls <- mclapply(1:iter, function (it){
    ...
    p <- sapply(1:n, function(j){
      lp <- pnorm(mean(score_exp[j,]), mean = score_back[j,], sd = sd(score_back)/2)
      return(2*(lp>0.5)*(1-lp) + 2*(lp<=0.5)*(lp))
    })
    ...
    p <- sapply(1:n, function(j) {
      if (sum(is.na(score_back[j,]))>1|sum(is.na(score_exp[j,]))>1) {
        return(1)} else {
          return(t.test(score_exp[j,!(is.na(score_exp[j,]))], rep(score_back[j,],2), alternative = "two.sided", var.equal = T)$p.value)}})
    discovery_ls = find_discovery_wpval(pval = p, q = q_tot)
...
saveRDS(sim_2sided_norm_homo, 'sim_2sided_unequal_norm_homo.rds')
...
saveRDS(sim_2sided_norm_hete, 'sim_2sided_unequal_norm_hete.rds')
...
mean_back = rep(20, n)
mean_exp = rep(20, n*2)
mean_exp[1:(length(positiveid)*2)] = rep(rpois(length(positiveid), lambda = 60), each = 2)
mean_exp[(length(negativeid)*2+1):(length(trueidx)*2)] = rep(rpois(length(negativeid), lambda = 5), each = 2)
...
saveRDS(sim_2sided_pois_homo, 'sim_2sided_unequal_pois_homo.rds')
...
saveRDS(sim_2sided_pois_hete, 'sim_2sided_unequal_pois_hete.rds')
...
mean_back = rep(30, n)
mean_exp = rep(30, n*2)
mean_exp[1:(length(positiveid)*2)] = rep(rnbinom(length(positiveid), size = 100, prob = nb.p), each = 2)
mean_exp[(length(negativeid)*2+1):(length(trueidx)*2)] = rep(pmax(1,rnbinom(length(negativeid), size = 7, prob = nb.p)), each = 2)
...
sim_2sided_nb_homo <- lapply(1:length(out_prop), function(i_out){
  ...
  fdppow_ls <- mclapply(1:iter, function (it){
    ...
    ##### use BH paired m2 misspecification as variance 1
    p <-  sapply(1:n, function(j){
      1 - pnbinom(sum(score_exp[j,]), size = score_back[j] * 2, nb.p)
    })
    p <- 2*(p>0.5)*(1-p) + 2*(p<=0.5)*(p)
    discovery_ls = find_discovery_wpval(pval = p, q = q_tot)
    fdppow_pair_2as1 = compute_fdppow(discovery_ls = discovery_ls, trueidx = trueidx)
...
saveRDS(sim_2sided_nb_homo, 'sim_2sided_unequal_nb_homo.rds')
...
saveRDS(sim_2sided_nb_hete, 'sim_2sided_unequal_nb_hete.rds')
