...
nonnull_prop = c(0.2, 0.4)

grid_ls = expand.grid(list(nonnull_prop, scenario_tot, dist_tot), stringsAsFactors = F)
...
for(i in 1:nrow(grid_ls)){
  ...
  scenario = grid_i[2]
  dist = grid_i[3]
  fdppow = readRDS(file = paste0('sim_multirep_',dist,'_',scenario,'_prop_interesting.rds'))
  dat = fdppow[[which(nonnull_prop %in% nonnull)]]
...
pdf(file =  'lineplot_enrichment_multirep_prop_interesting.pdf', height = 23.4, width = 15.3)
p = arrangeGrob(
  ...
  blankPlot,textGrob(label = 'p = 0.2                p = 0.4',hjust = 0.4, gp = gpar(fontsize= textsize)),blankPlot,textGrob(label = 'p = 0.2                p = 0.4',hjust = 0.5, gp = gpar(fontsize= textsize)),
