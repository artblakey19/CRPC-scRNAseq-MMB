remove(list = ls())
setwd('~/Dropbox/clipper/source042520/multiple/')
library(openxlsx)
library(scales)
library(ggplot2)
library(wesanderson)
...
grid_ls = expand.grid(list(cor_prop, scenario_tot, dist_tot), stringsAsFactors = F)

p_ls = vector(mode = 'list',length = 4)
...
for(i in 1:nrow(grid_ls)){
  ...
  scenario = grid_i[2]
  dist = grid_i[3]
  fdppow = readRDS(file = paste0('sim_multirep_',dist,'_',scenario,'_cor.rds'))
  dat = fdppow[[which(cor_prop %in% cor)]]
...
pdf(file =  'lineplot_enrichment_multirep_cor.pdf', height = 9, width = 15.3)
p = arrangeGrob(
  ncol = 4,nrow = 4, widths = c(0.5, 3, 0.5,3), heights = c(0.3,0.2, 0.3, 3),
  ...
  blankPlot,textGrob(label = 'Cor=0.2                 Cor=0.4',hjust = 0.4, gp = gpar(fontsize= textsize)),blankPlot,textGrob(label = 'Cor=0.2                 Cor=0.4',hjust = 0.5, gp = gpar(fontsize= textsize)),
  textGrob(label = 'Gaussian',just = 'left', gp = gpar(fontsize= titletextsize, fontface =2)), blankPlot, blankPlot, blankPlot,
