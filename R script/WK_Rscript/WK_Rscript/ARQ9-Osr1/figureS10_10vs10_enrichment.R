setwd('../data_simulation/10vs10_enrich/')
remove(list = ls())
library(parallel)
library(locfdr)
# library(truncnorm)
...
for(i in 1:nrow(grid_ls)){
  ...
  scenario = grid_i[2]
  dist = grid_i[3]
  fdppow = readRDS(file = paste0('sim_10rep_',dist,'_',scenario,'.rds'))
  dat = fdppow[[which(out_prop %in% cor)]]
  fdppow2 = readRDS(file = paste0('sim_10rep_',dist,'_',scenario,'_new.rds'))
  dat2 = fdppow2[[which(out_prop %in% cor)]]
  dat = rbind(dat, dat2)
  ####### clipper variants #####
  ylim_fdr = 0.10
  method_subset = c('Clipper',
                    'BH-pool', 'BH-pair-Wilcoxon','BH-pair-parametric','BH-pair-permutation',
                    'BH-pair-2as1','BH-pair-mis',
                    'locfdr-emp','locfdr-swap')
  
  dat$methods = format_method_name(dat$methods)
  dat = dat[dat$methods %in% method_subset ,]
  #### fdr
  p = ggplot(dat[dat$methods != 'Clipper' ,], 
  ...
  p =  p + geom_line(data=dat[dat$methods == "Clipper",], aes(q, y =  transformy(fdr) , col = methods), size = 1.5) + 
    geom_point(data=dat[dat$methods == "Clipper",], aes(q, y =  transformy(fdr), shape = methods, col = methods), size = 3)
  ...
  #### power
  p = ggplot(dat[dat$methods != 'Clipper' ,], aes(x = q, y = pow, group = methods)) +
    theme(text = element_text(size=textsize, color = 'black'),
          panel.grid = element_blank(),
          panel.background = element_blank(),
          axis.line.y = element_line(size = 0.5),
    ...
    scale_shape_manual(values = pt_shape[names(pt_shape) %in% method_subset]) 
  p =  p + geom_line(data=dat[dat$methods == "Clipper",], aes(q, y = pow, col = methods), size = 1.5) + geom_point(data=dat[dat$methods == "Clipper",], aes(q, y = pow, shape = methods, col = methods), size = 3)
  ## if the first plot
  if(i<=2 ){
    p = p + theme( axis.title.y = element_text()) + 
      ylab('Power (%)')
...
pdf(file =  'lineplot_enrichment_10rep_new.pdf', height = 16, width = 12.5)
p = arrangeGrob(
  ncol = 4,nrow = 6, widths = c(0.5, 3, 3,3), heights = c(0.3,0.3, 0.2, 3,0.2,3),
  blankPlot, textGrob(label = 'Gaussian',just = 'center', gp = gpar(fontsize= titletextsize, fontface =2)), textGrob(label = 'Poisson',just = 'center', gp = gpar(fontsize= titletextsize, fontface =2)), textGrob(label = 'Negative binomial',just = 'center', gp = gpar(fontsize= titletextsize, fontface =2)),
  blankPlot, textGrob(label = 'Heterogeneous',just = 'center', gp = gpar(fontsize= textsize)), textGrob(label = 'Heterogeneous',just = 'center', gp = gpar(fontsize= textsize)), textGrob(label = 'Heterogeneous',just = 'center', gp = gpar(fontsize= textsize)),
  textGrob(label = 'a Without outlier',just = 'left', gp = gpar(fontsize= titletextsize, fontface =2)), blankPlot, blankPlot,blankPlot,
  blankPlot, do.call('cbind', p_ls[1]),  do.call('cbind', p_ls[3]), do.call('cbind', p_ls[5]),
  textGrob(label = 'b With outliers',just = 'left', gp = gpar(fontsize= titletextsize, fontface =2)), blankPlot, blankPlot,blankPlot,
  blankPlot, do.call('cbind', p_ls[2]),  do.call('cbind', p_ls[4]), do.call('cbind', p_ls[6])
)
grid.arrange(p, legend, ncol = 2,nrow = 1, widths = c(9, 3.5))
dev.off()
