remove(list = ls())
setwd('../data_simulation/1vs1_enrich/')
library(parallel)
library(locfdr)
# library(truncnorm)
library(scales)
...
col_clipperdiff_rc = rgb(0, 114, 178, max = 255) # blue
col_clippermax_rc = rgb(86, 180, 233, max = 255)# sky blue 
col_clipperdiff_bh = rgb(0, 114, 178, max = 255) # blue 
col_clippermax_bh = rgb(86, 180, 233, max = 255)# sky blue 
...
pt_shape = c(1, 2,2,3,3,4:9)
names(pt_shape) = method_tot
mydarkgrey = "#252525"
myshade = alpha("#000000",alpha = 0.25)
display.brewer.all()
display.brewer.pal(13,'Greys')
brewer.pal(13,'Greys')
FDR  = 0.05
q_tot = (1:10)/100
textsize = 20
...
format_method_name = function(methods){
  ...
  methods[methods == 'pair-correct'] = 'BH-pair-correct'
...
grid_ls = expand.grid(list(dist_tot, scenario_tot, N), stringsAsFactors = F)
for(i in 1:nrow(grid_ls)){
  grid_i = grid_ls[i,]
  dist = grid_i[1]
  scenario = grid_i[2]
  n = grid_i[3]
  fdppow = readRDS(file = paste0('sim_singlerep_',dist,'_',scenario,'.rds'))
  dat = fdppow[[which(N %in% n)]]
  ...
  pdf(file = paste0('lineplot_fdr_singlerep_clippervariants_',dist,'_',scenario, '_n',n,'.pdf'), height = 4, width = 4.5)
  ...
  pdf(file = paste0('lineplot_pow_singlerep_clippervariants_',dist,'_',scenario, '_n',n,'.pdf'), height = 4, width = 4.5)
...
dat$methods = format_method_name(dat$methods)
pdf(file = paste0('lineplot_fdr_singlerep_clippervariants_',dist,'_',scenario, '_n',n,'_full.pdf'), height = 4, width = 4.5)
p = ggplot(dat[dat$methods %in% method_subset ,], 
           aes(x = q, y = fdr, group = methods)) +
  theme(text = element_text(size=textsize, color = 'black'),
        panel.grid = element_blank(),
...
for(i in 1:nrow(grid_ls)){
  ...
  fdppow = readRDS(file = paste0('sim_singlerep_',dist,'_',scenario,'.rds'))
  dat = Reduce(rbind.data.frame, fdppow[which(N %in% n)])
  dat$methods = format_method_name(dat$methods)
  dat$N = factor(rep(n, each = nrow(fdppow[[1]])))
  dat$fdr_sd  = dat$fdr_sd / sqrt(iter)
  dat$pow_sd  = dat$pow_sd/sqrt(iter)
  
  pdf(file = paste0('barplot_fdr_singlerep_',dist,'_', scenario,'.pdf'), height = 2, width = 8.5)
  p = ggplot(dat[dat$methods %in% method_subset & dat$q == FDR,],
    ...
    theme(text = element_text(size=textsize, color = 'black'),
          ...
          axis.title.y = element_blank()) +
    coord_cartesian( ylim=c(0, ylim_fdr)) +
    geom_bar(aes(x = N, fill = methods, y = pmin(ylim_fdr,fdr)),
             stat="identity",
             width = barwidth,
    ...
    scale_y_continuous(limits = c(0, 1),
                       ...
                       expand = c(0,0),
                       labels = function(x) paste0(x*100)) +
    scale_x_discrete(expand = c(0.22,0.22),label = paste0('d = ',n)) +
  ...
  ylim_pow = 1
  pdf(file = paste0('barplot_pow_singlerep_',dist,'_', scenario,'.pdf'), height = 2, width = 8.5)
  p = ggplot(dat[dat$methods %in% method_subset & dat$q == FDR,],
             aes(x = N, y = pow)) +
    theme(text = element_text(size=textsize, color = 'black'),
          panel.grid = element_blank(),
    ...
    geom_bar(aes(x = N, fill = methods, y = pmin(ylim_pow,pow)),
             stat="identity",
             width = barwidth,
             position = position_dodge(barwidth) )+
    geom_errorbar(width = 0.5,
