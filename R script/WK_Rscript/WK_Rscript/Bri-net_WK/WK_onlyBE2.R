...
DimPlot(onlyBE2, reduction = "tsne", pt.size = 1)
...
par(mfrow=c(2,3))
AUCell::AUCell_plotTSNE(tSNE_scenic$Y, exprMat, aucell_regulonAUC[onlyNonDuplicatedExtended(rownames(aucell_regulonAUC))[c("Ar_extended", "Fos_extended")],], plots="Expression")
...
write.csv(regulonActivity_byCellType_Scaled, file = "onlyBE2_TFs_SCENIC_Scaled.csv")
write.csv(regulonActivity_byCellType, file = "onlyBE2_TFs_SCENIC.csv")
...
write.csv(topRegulators, file = "onlyBE2_topRegulators_Scaled.csv")
topRegulators1 <- reshape2::melt(regulonActivity_byCellType)
colnames(topRegulators1) <- c("Regulon", "CellType", "RelativeActivity")
topRegulator1s <- topRegulators1[which(topRegulators1$RelativeActivity>0),]
viewTable(topRegulators1)
write.csv(topRegulators1, file = "onlyBE2_topRegulators.csv")
save.image("WK_onlyBE2.RData")
save.image("WK_onlyBE2.RData")