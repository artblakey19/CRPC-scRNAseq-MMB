...
###Double
##Double.BELEMETPos re-clustering
...
DimPlot(Double.combined1.BELEMETPos, reduction = "umap", label = TRUE)
...
#Cell Cycle regression
Double.combined1.BELEMETPos3 <- Double.combined1.BELEMETPos
DefaultAssay(Double.combined1.BELEMETPos3) <- "RNA"
Double.combined1.BELEMETPos3 <- ScaleData(Double.combined1.BELEMETPos3, vars.to.regress = c("S.Score", "G2M.Score"), features = rownames(Double.combined1.BELEMETPos3))
Double.combined1.BELEMETPos3 <- RunPCA(Double.combined1.BELEMETPos3, features = VariableFeatures(Double.combined1.BELEMETPos3))
ElbowPlot(Double.combined1.BELEMETPos3, ndims = 50)

Double.combined1.BELEMETPos3 <- FindNeighbors(Double.combined1.BELEMETPos3, reduction = "pca", dims = 1:14)
Double.combined1.BELEMETPos3 <- FindClusters(Double.combined1.BELEMETPos3, resolution = 0.5)
Double.combined1.BELEMETPos3 <- RunUMAP(Double.combined1.BELEMETPos3, reduction = "pca", dims = 1:14)

Idents(object = Double.combined1.BELEMETPos3) <- "EpiCellTypes"
DimPlot(Double.combined1.BELEMETPos3, reduction = "umap", pt.size = 0.3, label = TRUE)

##Convert Seurat to Monocle3 cell data set class
DefaultAssay(Double.combined1.BELEMETPos3) <- "RNA"
cds <- as.cell_data_set(Double.combined1.BELEMETPos3)

## Calculate size factors using built-in function in monocle3
cds <- estimate_size_factors(cds)

## Add gene names into CDS
cds@rowRanges@elementMetadata@listData[["gene_short_name"]] <- rownames(Double.combined1.BELEMETPos4)
...
tiff(file = "Double.combined1.BELEMETPos4 re-clustering trajectory UMAP.tiff", width = 6, height = 5, units = "in", compression = "lzw", res = 800)
plt + scale_color_manual(values = c("salmon", "skyblue1", "olivedrab2", "brown3", "deeppink1",  "blue", "darkorange", "red", "turquoise3",
                                    "bisque3", "slategray3", "mediumorchid3", 
                                    "yellow2", "green4", "black"))
dev.off()
...
tiff(file = "Double.combined1.BELEMETPos4 re-clustering pseudotime UMAP.tiff", width = 6, height = 5, units = "in", compression = "lzw", res = 800)
plt
dev.off()
...
tiff(file = "Double.combined1.BELEMETPos4 Tcf4 in pseudotime.tiff", width = 5, height = 4, units = "in", compression = "lzw", res = 1000)
plot_genes_in_pseudotime(Solid_lineage_cds, cell_size = 1.5,
                         color_cells_by="EpiCellTypes"
) + scale_color_manual(values = c("salmon", "skyblue1", "olivedrab2", "brown3", "deeppink1",  "blue", "darkorange", "red", "turquoise3",
                                  "bisque3", "slategray3", "mediumorchid3", 
