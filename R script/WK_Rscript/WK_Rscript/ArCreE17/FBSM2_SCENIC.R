...
DimPlot(FBSM2, reduction = "umap", pt.size = 1)
cellInfo <- data.frame(Class=Idents(FBSM2))
exprMat <- as.matrix(GetAssayData(FBSM2, slot = "counts"))
...
colVars <- list(CellType=c("mGFPPosArPos"="blue", 
                           "mGFPPosArNeg"="red",
                           "mGFPNegArNeg"="green"
))
...
myDatasetTitle="FBSM2 SCENIC RUN" # choose a name for your analysis
data(defaultDbNames)
...
write.csv(regulonActivity_byCellType_Scaled, file = "FBSM2_TFs_SCENIC.csv")
...
write.csv(topRegulators, file = "FBMS2_topRegulators.csv")
save.image("FBSM2.RData")
save.image("FBSM2.RData")