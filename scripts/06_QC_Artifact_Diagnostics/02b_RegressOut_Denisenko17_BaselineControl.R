...
# Stage 04 (which produced the baseline clustering) ran under
# plan("multicore", workers = 30). SCTransform draws an `ncells` subsample for its
# regularization fit, and the future RNG stream differs between sequential and
# multicore — so a sequential re-run yields a slightly different SCT model,
# residuals, PCA and Harmony embedding, and marginal cells flip clusters. That
# makes the no-regression "recompute" control fail to reproduce the baseline
# (ARI 0.778 instead of ~1), which destroys the arm2-vs-arm3 contrast this script
# exists to measure. Match Stage 04's execution environment so the control is a
# true replication.
